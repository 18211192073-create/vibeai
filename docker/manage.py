...
def show_files():
    ...
    assistant_dir = output_dir / "assistant"
    if assistant_dir.exists():
        assistant_files = sorted(assistant_dir.glob("*"), key=lambda x: x.stat().st_mtime, reverse=True)
        if assistant_files:
            print(f"  🧠 AI 日报助手 (assistant/): {len(assistant_files)} 个文件")
            for file in assistant_files[:5]:
                mtime = time.ctime(file.stat().st_mtime)
                size_kb = file.stat().st_size // 1024
                print(f"    📄 {file.name} ({size_kb}KB, {mtime.split()[3][:5]})")
...
def _is_expected_webserver_process(pid: int) -> bool:
    ...
    if not cmdline:
        return False
    return (
        ("trendradar.assistant.web" in cmdline or "http.server" in cmdline)
        and str(WEBSERVER_PORT) in cmdline
    )
...
def start_webserver(force: bool = False):
    ...
    print(f"  🔒 安全提示：提供日报页面与本地 API，默认限制在助手数据目录")
    ...
    try:
        # 启动 AI 行业日志助手 Web 服务器
        process = subprocess.Popen(
            [sys.executable, '-m', 'trendradar.assistant.web', '--host', '0.0.0.0', '--port', str(WEBSERVER_PORT)],
            cwd="/app",
        ...
        if process.poll() is None:
            ...
            print(f"  📁 服务目录: output/assistant 与 output 文件")
...
📋 命令列表:
  ...
  logs             - 实时查看日志
  restart          - 重启说明
  start_webserver  - 启动 AI 行业日报网页服务
  stop_webserver   - 停止网页服务
  webserver_status - 查看网页服务状态
  help             - 显示此帮助
