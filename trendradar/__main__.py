...
from trendradar.assistant import build_daily_digest, load_assistant_settings, send_feishu_digest, AssistantStorage
from trendradar.assistant.web import run_server as run_assistant_web_server
...
class NewsAnalyzer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.proxy_url = None
        self.assistant_settings = load_assistant_settings()
        self._setup_proxy()
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self._maybe_run_ai_assistant_digest()
    ...
    def _maybe_run_ai_assistant_digest(self, force: bool = False) -> Optional[Dict]:
        """在指定时间生成 AI 行业日报。"""
        if not self.assistant_settings.get("enabled", True) and not force:
            print("[DAY VIBE AI] 已禁用，跳过日报生成")
            return None
        ...
        if not force and current_hhmm != report_time:
            print(f"[DAY VIBE AI] 当前时间 {current_hhmm} 未到日报时间 {report_time}，跳过")
            return None

        print("[DAY VIBE AI] 开始生成日报...")
        ...
        print(
            f"[DAY VIBE AI] 已生成日报: {report.get('title')}，"
            f"选中 {len(report.get('items', []))} 条，保存到 output/assistant"
        )

        feishu_cfg = self.assistant_settings.get("feishu", {})
        if feishu_cfg.get("enabled") and feishu_cfg.get("webhook_url"):
            sent = send_feishu_digest(
                feishu_cfg.get("webhook_url", ""),
                report,
                dashboard_url=feishu_cfg.get("dashboard_url", ""),
            )
            print(f"[DAY VIBE AI] 飞书推送: {'成功' if sent else '失败'}")
        else:
            print("[DAY VIBE AI] 未配置飞书 webhook，跳过推送")
...
def _build_test_report_data(ctx: AppContext) -> Dict:
    ...
    title = f"DAY VIBE AI 通知测试消息（{now.strftime('%Y-%m-%d %H:%M:%S')}）"
...
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>DAY VIBE AI 通知测试</title></head>
<body>
<h2>DAY VIBE AI 通知连通性测试</h2>
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DAY VIBE AI - 热点新闻聚合与分析工具",
...
"""
    ...
    try:
        ...
        if args.assistant_web:
            run_assistant_web_server()
            return
        ...
        if args.ai_digest:
            analyzer._maybe_run_ai_assistant_digest(force=True)
            return
