...
class RSSParser:
    ...
    def parse(self, content: str, feed_url: str = "") -> List[ParsedRSSItem]:
        ...
        if "anthropic.com/news" in (feed_url or ""):
            anthropic_items = self._parse_anthropic_news_page(content, feed_url)
            if anthropic_items:
                return anthropic_items
    ...
    def _parse_anthropic_news_page(self, content: str, feed_url: str = "") -> List[ParsedRSSItem]:
        """从 Anthropic newsroom HTML 页面提取新闻条目。"""
        article_urls = []
        for match in re.findall(r'https://www\.anthropic\.com/news/[^\s"\'<>?#]+', content or ""):
            normalized = match.rstrip("/").rstrip(".")
            if normalized not in article_urls and normalized != "https://www.anthropic.com/news":
        ...
        for url in article_urls[:24]:
            try:
                ...
                response = requests.get(
                    ...
                    timeout=10,
                    headers={
                        "User-Agent": "TrendRadar/2.0 RSS Reader (Anthropic HTML fallback)",
                ...
                title = self._extract_meta(article_html, ["og:title", "twitter:title"]) or self._extract_title(article_html)
                if not title:
                    continue
                ...
                summary = self._extract_meta(article_html, ["og:description", "twitter:description"]) or ""
                summary = self._clean_text(summary)
                if summary and len(summary) > self.max_summary_length:
                    summary = summary[: self.max_summary_length] + "..."
                published_at = self._extract_meta(article_html, ["article:published_time", "og:updated_time"]) or None
                if published_at:
                    published_at = self._parse_iso_date(published_at)
                items.append(
                    ParsedRSSItem(
                        title=title,
                        url=url,
    ...
    def _extract_meta(self, html_text: str, names: List[str]) -> str:
        for name in names:
            patterns = [
