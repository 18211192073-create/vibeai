    ...
    def trigger_crawl(self, platforms: Optional[List[str]] = None, save_to_local: bool = False, include_url: bool = False) -> Dict:
        ...
        try:
            ...
            try:
                ...
                rss_config = config_data.get("rss", {}) or config_data.get("RSS", {})
                if rss_config.get("enabled", rss_config.get("ENABLED", False)):
                    try:
                        ...
                        if save_to_local:
                            rss_saved = storage.save_rss_data(rss_data)
                        else:
                            rss_saved = False
                        rss_total = sum(len(items) for items in rss_data.items.values())
                        rss_result = {
                            "success": True,
                            "saved_to_local": bool(save_to_local and rss_saved),
                            "total_items": rss_total,
                            "failed_ids": rss_data.failed_ids,
                            "id_to_name": rss_data.id_to_name,
                        }
                        print(f"[RSS] 临时爬取完成：{rss_total} 条，保存到本地：{bool(save_to_local and rss_saved)}")
                    except Exception as rss_exc:
                        rss_result = {
                            "success": False,
                            "error": str(rss_exc),
                        }
                        print(f"[RSS] 临时爬取失败: {rss_exc}")
            finally:
                get_cache().clear()
                print("[System] 缓存已清除")
                storage.cleanup()
